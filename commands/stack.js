...
module.exports = {
    ...
    async execute(interaction) {
        ...
        for (let i = 1; i < (numPlayers + 1); i++) {
            const currentPlayer = await interaction.guild.members.fetch(interaction.options.getUser("p" + i).id)
        ...
        for (player of shuffledArray) {
            objectArray.push({ player: player, position: "Has not picked yet" })
        }
...
async function badabingBadaboom(objectArray, interaction, pickTime, recentlyPicked) {
    const updatedArray = []
    ...
    const randomRole = shuffle(availableRoles(updatedArray))[0]
    if (nextUp) {
        await interaction.edit({
            content: `${nextUp.player.toString()} You're up! If you do not pick you will be assigned ${randomRole}`, embeds: [embed.embed], components: buttonRows
            , files: [embed.file]
        });
    } else {
        await interaction.edit({
            content: "", embeds: [embed.embed], files: [embed.file], components: []
        });
        return
    }
    ...
    const collector = interaction.channel.createMessageComponentCollector({ filter, time: pickTime, max: 1 });
    ...
    collector.on('end', async collected => {
        if (collected.last()) {
            try {
                const recentlyPicked = { player: nextUp.player, position: collected.last().customId }
                await badabingBadaboom(updatedArray, interaction, pickTime, recentlyPicked)
            }
            catch (error) {
...
async function prettyEmbed(playerArray) {
    const BLANK = "\u200b"
    const playerFields = [{
        name: "Picking order: ", value: arrayPrettifier(playerArray).join('\n'),
    },]
...
function arrayPrettifier(playerArray) {
    ...
    for (player of playerArray) {
        if (player.player.nickname) {
            const neededFilling = optimalStringLength - (player.player.nickname.length + player.position.length)
            prettyArray.push(stringPrettifier(player.player.nickname, neededFilling, player.position))
        } else {
            const neededFilling = optimalStringLength - (player.player.user.username.length + player.position.length)
            prettyArray.push(stringPrettifier(player.player.user.username, neededFilling, player.position))
        }
    }
    return prettyArray
}
...
async function artTime(objectArray) {
    ...
    for (object of objectArray) {
        if (object.position.startsWith("pos")) {
            const { body } = await request(object.player.user.displayAvatarURL({ extension: 'jpg' }));
    ...
    const attachment = new AttachmentBuilder(await canvas.encode('png'), { name: 'dota-map.png' });
    return attachment
}
...
function availableRoles(objectArray) {
    const available = ["pos1", "pos2", "pos3", "pos4", "pos5"]
    for (object of objectArray) {
        if (object.position.startsWith("pos")) {
            available.splice(available.indexOf(object.position), 1)
        }
    }
    return available
}